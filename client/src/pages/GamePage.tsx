...
import type { FullList, Team, RevealedItem, GuessResult, GameState } from '../types';
...
export default function GamePage() {
  ...
  const [teams, setTeams] = useState<Team[]>(() =>
    state?.teamNames.map((name, i) => ({ name, color: TEAM_COLORS[i % TEAM_COLORS.length], score: 0 })) ?? []
  );
  const [activeTeamIndex, setActiveTeamIndex] = useState(0);
  const [guessesUsed, setGuessesUsed] = useState<number[]>(() =>
    new Array(state?.teamNames.length ?? 0).fill(0)
  );
  ...
  const [suggestions, setSuggestions] = useState<import('../types').ListItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchList(state.slug)
      .then((l) => { setList(l); initMatcher(l.items); })
  ...
  if (error || !list) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <button onClick={() => navigate('/')} style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text)', padding: '8px 16px', borderRadius: 4, cursor: 'pointer' }}>Back</button>
      </div>
    </div>
  );
          ...
          return (
            <button key={team.name} onClick={() => setActiveTeamIndex(i)} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                ...
                <span style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 0.5, color: isActive ? team.color : 'var(--text-2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{team.name}</span>
              </div>
      ...
      <div style={{ flexShrink: 0, padding: '12px 14px', borderBottom: '1px solid var(--border)' }}>
              ...
              {suggestions.map((item, i) => (
                <div key={item.rank} onMouseDown={() => submitGuess(item.value)} style={{
                  ...
                  <span style={{
                    ...
                    color: i === selectedSuggestion ? 'rgba(0,0,0,0.6)' : 'var(--text-3)',
                    minWidth: 24, textAlign: 'right',
                  }}>{item.rank}</span>
                  <span style={{
                    fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 700, fontSize: 15,
                    color: i === selectedSuggestion ? '#000' : 'var(--text)', flex: 1,
                  }}>{item.value}</span>
                  {item.hint && (
        ...
        {lastResult && (
          <div style={{
            ...
            color: lastResult.matched ? 'var(--green)' : 'var(--red)',
            letterSpacing: 0.5,
          }}>
            {lastResult.matched
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {recentGuesses.map((g, i) => (
              <div key={i} style={{
                ...
                {g.matched && (
                  <span style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 700, fontSize: 14, color: 'var(--green)' }}>+{g.points}</span>
                )}
                {g.teamName && (
                  <span style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 12, color: 'var(--text-3)' }}>{g.teamName}</span>
                )}
              </div>
            ))}
          </div>
